...
export default function CustomerMenuPage() {
	...
	useEffect(() => {
		const delayDebounce = setTimeout(() => {
			if (searchText.trim() !== '') {
				...
				callApiMenuCus(
					`${REACT_APP_API_URL}/menu/cus/menus/${encodeURIComponent(
						searchText.trim(),
					)}`,
				);
			} else {
	...
	async function callApiMenuByCategory(id) {
		try {
			...
			const response = await axios.get(
				`${REACT_APP_API_URL}/menu/cus/menus/category/${id}`,
			);
			setFoods(response.data.data);
		} catch (err) {
			console.error('API GET error:', err);
	...
	const handleSetCart = (food) => {
		...
		sessionStorage.setItem(
			'order',
			JSON.stringify({ orderId: tableId, foodOrderList: order.foodOrderList }),
		);
		...
		<Layout style={{ minHeight: '100vh', background: '#fafafa' }}>
			...
			>
				...
				<div style={{ padding: '0 12px 12px 12px' }}>
					<Input
						placeholder={
							isSearching ? 'Đang tìm kiếm...' : 'Tìm món ăn, đồ uống...'
						}
						prefix={
							<SearchOutlined
								style={{ color: isSearching ? '#226533' : '#7f8c8d' }}
							/>
						}
						value={searchText}
								...
								style={{
									...
									background:
										selectedCategory === 'all'
											...
											: '#fff',
									borderColor:
										selectedCategory === 'all' ? 'transparent' : '#d9d9d9',
									boxShadow:
										selectedCategory === 'all'
									...
									style={{
										...
										background:
											selectedCategory === cat.id
												...
												: '#fff',
										borderColor:
											selectedCategory === cat.id ? 'transparent' : '#d9d9d9',
										boxShadow:
											selectedCategory === cat.id
												? '0 2px 8px rgba(34, 101, 51, 0.2)'
												: '0 1px 3px rgba(0,0,0,0.1)',
