...
export default function HomecsPage() {
	...
	const handleCallStaff = async () => {
		try {
			...
			if (!qrSessionId) {
				message.error(
					'Không tìm thấy thông tin phiên QR. Vui lòng quét mã QR lại.',
				);
			...
			const response = await axios.post(
				`${process.env.REACT_APP_API_URL}/call-staff`,
				{
			...
			message.error(
				error.response?.data?.error ||
					'Có lỗi xảy ra khi gọi nhân viên. Vui lòng thử lại.',
			);
		} finally {
			setIsLoading(false);
		}
		...
		<Layout style={{ minHeight: '100vh', background: '#fafafa' }}>
			...
			>
				...
				<Text style={{ fontSize: 18, color: '#666' }}>
					Số 163 Mai Hắc Đế, phường Nguyễn Du, quận Hai Bà Trưng
				</Text>
