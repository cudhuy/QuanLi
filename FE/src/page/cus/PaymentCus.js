...
export default function PaymentPage() {
	...
	const { totalPrice: initialTotal = 0, unpaidOrders = [] } =
		location.state || {};
	...
	const handleCashPayment = async () => {
		try {
			...
			const notificationData = {
				...
				message: `Khách hàng ở bàn ${table_id} yêu cầu thanh toán ${formatPrice(
					finalAmount,
				)}đ bằng tiền mặt.`,
		...
		<Layout style={{ minHeight: '100vh', background: '#f5f7fa' }}>
			...
			>
				...
				<div className='text-center'>
					<Title
						level={5}
						style={{ margin: 0, color: '#226533', fontWeight: 600 }}
					>
						Thanh toán
					</Title>
				</div>
			...
			>
				...
				>
					...
					<Text
						strong
						style={{ fontSize: 16, color: '#226533', fontWeight: 600 }}
					>
					...
					>
						{paymentMethods.map((method) => (
							<div
								...
								style={{
									border: `2px solid ${
										paymentMethod === method.key ? '#226533' : '#e8e8e8'
									}`,
									borderRadius: 8,
									padding: '16px 12px',
									display: 'flex',
