...
const TablesPage = () => {
    ...
    `;
        ...
        `;
			...
			handleSimplePrint(
				fallbackContent,
				`In QR Code - Bàn ${table.table_number}`,
			);
		}
	};
	...
	const handlePrintAllQR = () => {
		...
		tablesWithQR.forEach((table) => {
			const qrUrl = `${replaceUrlServer(REACT_APP_API_URL)}${
				table.qr_code_url
			}`;
			qrItems += `
        <div class="qr-container">
	...
	const columns = [
		...
		},
		{
			...
			align: 'center',
			sorter: (a, b) =>
				(a.table_number || '').localeCompare(b.table_number || '', 'vi'),
		},
		{
			title: 'Trạng thái',
			dataIndex: 'is_active',
						...
						>
							...
							<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
								<Button
									...
									disabled={
										filteredTables.filter((t) => t.qr_code_url).length === 0
									}
								>
									🖨️ In tất cả QR
								</Button>
						...
						<Form form={addForm} layout='vertical' initialValues={{}}>
							<Form.Item
								...
								rules={[
									...
									{
										pattern: /^[A-Za-z0-9\s]+$/,
										message: 'Số bàn chỉ chứa chữ, số và khoảng trắng!',
									},
								]}
							>
							...
							{editingTable?.qr_code_url && (
								<Form.Item label='QR Code hiện tại'>
									<div style={{ textAlign: 'center' }}>
										<img
											src={`${replaceUrlServer(REACT_APP_API_URL)}${
												editingTable.qr_code_url
											}`}
											alt='Current QR'
											style={{
