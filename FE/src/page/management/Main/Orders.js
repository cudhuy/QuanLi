...
const OrderPage = () => {
	...
	const [filterStatus, setFilterStatus] = useFilterState(
		'orders_filterStatus',
		'all',
	);
	const [filterTime, setFilterTime] = useFilterState(
		'orders_filterTime',
		'today',
	);
	...
	const [modalHoanDon, setModalHoanDon] = useState({
		open: false,
		order: null,
	});
	...
	const addItemToNewOrder = () => {
		if (!newItem.name || !newItem.price)
			return message.warning('Chọn món trước!');
		setNewOrder((prev) => ({
			...prev,
	...
	const handleCreateOrder = () => {
		...
		if (newOrder.items.length === 0)
			return message.error('Đơn hàng chưa có món!');
			...
			return (
				<>
					<Button
						...
						style={{ marginRight: 8 }}
						onClick={() =>
							setModalHoanDon({ open: true, order: selectedOrder })
						}
					>
						Hoàn đơn
					</Button>
			...
			return (
				<>
					...
					<Button
						...
						onClick={() =>
							setModalThanhToan({ open: true, order: selectedOrder })
						}
					>
						Thanh toán
					</Button>
	...
	return (
		<>
					...
					>
						<Spin spinning={loading} tip='Đang tải dữ liệu...'>
							...
							<div style={{ flex: 1, overflow: 'auto', padding: '20px' }}>
								...
								>
									<span>
										Hiển thị 1 đến {pagedOrders.length} trong tổng số{' '}
							...
							>
								<p>
									Bạn có chắc chắn muốn xuất danh sách đơn hàng ra file Excel
									không?
								</p>
							</Modal>
						</Spin>
			...
			<Drawer
				...
				footer={
					<div style={{ textAlign: 'right' }}>{renderDrawerFooter()}</div>
				}
			>
				{selectedOrder && (
					<>
			...
			<Drawer
				...
				footer={
					<div style={{ textAlign: 'right' }}>
						<Button
							onClick={() => setNewOrderDrawer(false)}
							style={{ marginRight: 8 }}
						>
							Hủy
						</Button>
			...
			<Modal
				...
				footer={[
					<Button
						key='back'
						onClick={() => setModalHuy({ open: false, order: null })}
					>
						Đóng
					</Button>,
					<Button
			...
			<Modal
				...
				footer={[
					<Button
						key='back'
						onClick={() => setModalThanhToan({ open: false, order: null })}
					>
						Đóng
					</Button>,
					<Button
			...
			>
				<p>
					Bạn có chắc muốn thanh toán đơn {modalThanhToan.order?.code} không?
				</p>
			</Modal>
			...
			<Modal
				...
				footer={[
					<Button
						key='back'
						onClick={() => setModalHoanDon({ open: false, order: null })}
					>
						Đóng
					</Button>,
					<Button
